...
use crate::python_graph_capture::{
    coordinator_python_capture_enabled, initialize_spark_python_capture_from_env,
    launch_python_graph_capture, spark_python_capture_enabled, PythonDeviceBufferArg,
    PythonGraphCaptureLaunch, PythonKernelArg,
};
use anyhow::{Context, Result};
use glmrt_core::GLM52_ROUTED_SCALING_FACTOR;
use glmrt_ffi::{
    GlmrtB12xSparkMlpBuffers, GlmrtCudaGraphCaptureInfo, GlmrtDeviceBuffer, GlmrtHostBuffer,
    NativeLibrary, GLMRT_CUDA_ROUTER_TOPK_MAX_K, GLMRT_CUDA_SAMPLE_TOPK_MAX_K,
};
use serde_json::json;
use std::{
    env,
...
const KERNEL_FILTERS: &[&str] = &[
    ...
    "spark",
    "spark-dense-gemm",
    "b12x_spark_dense_gemm_fp4",
    "spark-single-expert-mlp",
    "b12x_spark_single_expert_mlp_fp4",
    "spark-concurrent-expert-mlp",
    "b12x_spark_aot_concurrent_expert_mlp_fp4",
];
...
pub(crate) fn run_bench_cuda_kernels(args: BenchCudaKernelsArgs) -> Result<()> {
    ...
    let spark_selected = kernel_selected(
        &args,
        "b12x_spark_dense_gemm_fp4",
        &["spark", "spark-dense-gemm"],
    ) || kernel_selected(
        &args,
        "b12x_spark_single_expert_mlp_fp4",
        &["spark", "spark-single-expert-mlp"],
    );
    if spark_python_capture_enabled() && spark_selected {
        ...
        if kernel_selected(
            ...
            bench_b12x_spark_dense_gemm(&library, &stream, &native_lib, &args)?;
        }
        if kernel_selected(
            &args,
            "b12x_spark_single_expert_mlp_fp4",
            ...
            bench_b12x_spark_single_expert_mlp(&library, &stream, &native_lib, &args)?;
        }
    }
    if kernel_selected_explicit(
        &args,
        ...
        bench_b12x_spark_aot_concurrent_expert_mlp(&library, &native_lib, &args)?;
    }
    Ok(())
}
...
fn bench_b12x_spark_dense_gemm(
    library: &NativeLibrary,
    stream: &CudaStream<'_>,
    native_lib: &Path,
    args: &BenchCudaKernelsArgs,
    ...
    if args.hidden_dim % 16 != 0 {
        anyhow::bail!(
            "b12x Spark dense GEMM benchmark requires hidden_dim divisible by 16, got {}",
            args.hidden_dim
        );
    }
    if args.output_dim == 0 {
        anyhow::bail!("b12x Spark dense GEMM benchmark requires nonzero output_dim");
    }

    for rows in [args.rows] {
        let output_values = rows
            .checked_mul(args.output_dim)
            .context("b12x Spark dense GEMM output value count overflow")?;
        let out_buffer = DeviceAllocation::new(
            library,
            output_values * 2,
            "b12x Spark dense GEMM BF16 output",
        )?;
        let output_buffer = out_buffer.buffer();
        ...
        let kwargs = [
            ...
            ("n", PythonKernelArg::Usize(args.output_dim)),
            ("k", PythonKernelArg::Usize(args.hidden_dim)),
            ("seed", PythonKernelArg::Usize(42)),
            ("validate", PythonKernelArg::Bool(true)),
        ];
        ...
        .with_context(|| format!("warming b12x Spark dense GEMM rows={rows}"))?;
        unsafe {
            stream
                .synchronize()
                .context("synchronizing b12x Spark dense GEMM warmup")?;
            library
                .cuda_graph_begin_capture(stream.raw())
                .context("beginning b12x Spark dense GEMM CUDA graph capture")?;
            launch_python_graph_capture(PythonGraphCaptureLaunch {
                module: "b12x_spark_capture",
                function: "capture_dense_gemm",
                cuda_stream: stream.raw(),
            ...
            .with_context(|| format!("capturing b12x Spark dense GEMM rows={rows}"))?;
            let graph = BenchCudaGraph::new(
                library,
                library
                    .cuda_graph_end_capture_retained(stream.raw())
                    .context("ending b12x Spark dense GEMM CUDA graph capture")?,
            )?;
            let timing = time_kernel(args, || unsafe {
                library
                    .cuda_graph_launch(graph.graph_exec(), stream.raw())
                    .context("launching b12x Spark dense GEMM CUDA graph")?;
                stream.synchronize()
            })?;
            emit_timing(
                native_lib,
                ...
                &json!({
                    ...
                    "synthetic_nvfp4_validated": true,
                    "software_decode_validated": true,
                    "software_decode_rtol": 1.0e-3,
                    "software_decode_atol": 1.0e-2,
                    "rust_output_buffer": true,
                    "modelopt_scale_bridge": true,
                    "logical_payload_bytes": rows * args.hidden_dim / 2
                        + args.output_dim * args.hidden_dim / 2
                        + output_values * 2,
                }),
                args,
                timing,
...
fn bench_b12x_spark_single_expert_mlp(
    library: &NativeLibrary,
    stream: &CudaStream<'_>,
    native_lib: &Path,
    args: &BenchCudaKernelsArgs,
) -> Result<()> {
    if args.hidden_dim % 16 != 0 || args.intermediate_dim % 16 != 0 {
        anyhow::bail!(
            "b12x Spark single-expert MLP benchmark requires hidden_dim and intermediate_dim divisible by 16, got hidden_dim={} intermediate_dim={}",
            args.hidden_dim,
            args.intermediate_dim
        );
    }
    ...
    for rows in [args.rows] {
        let input_values = rows
            .checked_mul(args.hidden_dim)
            .context("b12x Spark single-expert MLP input value count overflow")?;
        let output_values = rows
            .checked_mul(args.output_dim)
            .context("b12x Spark single-expert MLP output value count overflow")?;
        let input = bf16_pattern(input_values);
        let input_buffer = upload(
            library,
            u16_bytes(&input),
            "b12x Spark single-expert MLP BF16 input",
        )?;
        let out_buffer = DeviceAllocation::new(
            library,
            output_values * 2,
            "b12x Spark single-expert MLP BF16 output",
        )?;
        let output_buffer = out_buffer.buffer();
        let input_buffer_arg = input_buffer.buffer();
        let buffers = [
            PythonDeviceBufferArg {
                name: "input",
                ptr: input_buffer_arg.ptr,
                bytes: input_buffer_arg.bytes,
                device_id: input_buffer_arg.device_id,
                flags: input_buffer_arg.flags,
            },
            PythonDeviceBufferArg {
                name: "output",
                ptr: output_buffer.ptr,
        ...
        let kwargs = [
            ...
            ("output", PythonKernelArg::Usize(args.output_dim)),
            ("seed", PythonKernelArg::Usize(43)),
            ("validate", PythonKernelArg::Bool(true)),
        ];

        launch_python_graph_capture(PythonGraphCaptureLaunch {
            module: "b12x_spark_capture",
            function: "capture_single_expert_mlp",
            cuda_stream: stream.raw(),
            buffers: &buffers,
            kwargs: &kwargs,
        })
        .with_context(|| format!("warming b12x Spark single-expert MLP rows={rows}"))?;
        unsafe {
            stream
                .synchronize()
                .context("synchronizing b12x Spark single-expert MLP warmup")?;
            library
                .cuda_graph_begin_capture(stream.raw())
                .context("beginning b12x Spark single-expert MLP CUDA graph capture")?;
            launch_python_graph_capture(PythonGraphCaptureLaunch {
                module: "b12x_spark_capture",
                function: "capture_single_expert_mlp",
                cuda_stream: stream.raw(),
            ...
            .with_context(|| format!("capturing b12x Spark single-expert MLP rows={rows}"))?;
            let graph = BenchCudaGraph::new(
                library,
                library
                    .cuda_graph_end_capture_retained(stream.raw())
                    .context("ending b12x Spark single-expert MLP CUDA graph capture")?,
            )?;
            let timing = time_kernel(args, || unsafe {
                library
                    .cuda_graph_launch(graph.graph_exec(), stream.raw())
                    .context("launching b12x Spark single-expert MLP CUDA graph")?;
                stream.synchronize()
            })?;
            emit_timing(
                native_lib,
                ...
                &json!({
                    ...
                    "software_decode_validated": true,
                    "software_decode_rtol": 1.0e-3,
                    "software_decode_atol": 2.0e-2,
                    "captures_input_quant": true,
                    "rust_input_buffer": true,
                    "rust_output_buffer": true,
                    "modelopt_scale_bridge": true,
                    "logical_payload_bytes": rows * args.hidden_dim / 2
                        + (2 * args.intermediate_dim) * args.hidden_dim / 2
                        + rows * args.intermediate_dim / 2
                        + args.output_dim * args.intermediate_dim / 2
                        + output_values * 2,
                }),
                args,
                timing,
...
struct B12xAotBenchExpert<'a> {
    _allocations: Vec<DeviceAllocation<'a>>,
    buffers: GlmrtB12xSparkMlpBuffers,
}

fn allocate_zeroed_b12x_buffer<'a>(
    library: &'a NativeLibrary,
    allocations: &mut Vec<DeviceAllocation<'a>>,
    bytes: usize,
    label: &str,
) -> Result<GlmrtDeviceBuffer> {
    let allocation = DeviceAllocation::new(library, bytes, label)?;
    library
        .cuda_zero_bytes(allocation.buffer(), bytes)
        .with_context(|| format!("zeroing {label}"))?;
    let buffer = allocation.buffer();
    allocations.push(allocation);
    Ok(buffer)
}

fn aligned_b12x_scale_bytes(rows: usize, cols: usize) -> Result<usize> {
    let rows = rows
        .checked_add(127)
        .context("B12X scale row alignment overflow")?
        / 128
        * 128;
    let scale_cols = cols / 16;
    let scale_cols = scale_cols
        .checked_add(3)
        .context("B12X scale column alignment overflow")?
        / 4
        * 4;
    rows.checked_mul(scale_cols)
        .context("B12X scale byte count overflow")
}

fn allocate_b12x_aot_bench_expert<'a>(
    library: &'a NativeLibrary,
    rows: usize,
    hidden_dim: usize,
    intermediate_dim: usize,
    output_dim: usize,
) -> Result<B12xAotBenchExpert<'a>> {
    let mut allocations = Vec::new();
    let matrix_bytes = |rows: usize, cols: usize, divisor: usize| -> Result<usize> {
        rows.checked_mul(cols)
            .and_then(|values| values.checked_mul(2))
            .map(|bytes| bytes / divisor)
            .context("B12X benchmark matrix byte count overflow")
    };
    let gate_weight = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        matrix_bytes(intermediate_dim, hidden_dim, 4)?,
        "B12X benchmark gate weight",
    )?;
    let gate_scale = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        aligned_b12x_scale_bytes(intermediate_dim, hidden_dim)?,
        "B12X benchmark gate scale",
    )?;
    let up_weight = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        matrix_bytes(intermediate_dim, hidden_dim, 4)?,
        "B12X benchmark up weight",
    )?;
    let up_scale = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        aligned_b12x_scale_bytes(intermediate_dim, hidden_dim)?,
        "B12X benchmark up scale",
    )?;
    let down_weight = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        matrix_bytes(output_dim, intermediate_dim, 4)?,
        "B12X benchmark down weight",
    )?;
    let down_scale = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        aligned_b12x_scale_bytes(output_dim, intermediate_dim)?,
        "B12X benchmark down scale",
    )?;
    let output = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        matrix_bytes(rows, output_dim, 1)?,
        "B12X benchmark output",
    )?;
    let input_packed = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        matrix_bytes(rows, hidden_dim, 4)?,
        "B12X benchmark packed input",
    )?;
    let input_scale = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        aligned_b12x_scale_bytes(rows, hidden_dim)?,
        "B12X benchmark input scale",
    )?;
    let gate_output = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        matrix_bytes(rows, intermediate_dim, 1)?,
        "B12X benchmark gate output",
    )?;
    let up_output = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        matrix_bytes(rows, intermediate_dim, 1)?,
        "B12X benchmark up output",
    )?;
    let activation_packed = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        matrix_bytes(rows, intermediate_dim, 4)?,
        "B12X benchmark packed activation",
    )?;
    let activation_scale = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        aligned_b12x_scale_bytes(rows, intermediate_dim)?,
        "B12X benchmark activation scale",
    )?;
    let alphas = allocate_zeroed_b12x_buffer(
        library,
        &mut allocations,
        3 * std::mem::size_of::<f32>(),
        "B12X benchmark alphas",
    )?;
    Ok(B12xAotBenchExpert {
        _allocations: allocations,
        buffers: GlmrtB12xSparkMlpBuffers {
            input: GlmrtDeviceBuffer::default(),
            gate_weight,
            gate_scale,
            up_weight,
            up_scale,
            ...
            activation_packed,
            activation_scale,
            gate_scale_swizzled: gate_scale,
            up_scale_swizzled: up_scale,
            down_scale_swizzled: down_scale,
            alphas,
        },
    })
}

fn bench_b12x_spark_aot_concurrent_expert_mlp(
    library: &NativeLibrary,
    native_lib: &Path,
    args: &BenchCudaKernelsArgs,
) -> Result<()> {
    anyhow::ensure!(
        args.rows <= 256,
        "B12X AOT benchmark supports at most 256 rows"
    );
    anyhow::ensure!(
        args.hidden_dim % 16 == 0 && args.intermediate_dim % 16 == 0,
        "B12X AOT benchmark dimensions must be divisible by 16"
    );
    anyhow::ensure!(
        library.cuda_b12x_spark_aot_available()?,
        "B12X Spark AOT kernels are unavailable"
    );
    library.cuda_b12x_spark_aot_init()?;

    let max_streams = args.routes.min(8);
    let stream_counts = [1_usize, 2, 4, 8]
        .into_iter()
        .filter(|count| *count <= max_streams)
        .collect::<Vec<_>>();
    anyhow::ensure!(
        !stream_counts.is_empty(),
        "B12X AOT benchmark requires routes >= 1"
    );
    let streams = (0..max_streams)
        .map(|_| CudaStream::new(library))
        .collect::<Result<Vec<_>>>()?;
    let experts = (0..max_streams)
        .map(|_| {
            allocate_b12x_aot_bench_expert(
                library,
                args.rows,
                args.hidden_dim,
                args.intermediate_dim,
        ...
        .collect::<Result<Vec<_>>>()?;
    let completion_stream = CudaStream::new(library)?;
    let zero_ready = CudaEvent::new(library)?;
    let expert_ready = (0..max_streams)
        .map(|_| CudaEvent::new(library))
        .collect::<Result<Vec<_>>>()?;
    let source_row_stride_bytes = args
        .hidden_dim
        .checked_div(2)
        .and_then(|packed| packed.checked_add(args.hidden_dim / 16))
        .context("B12X terminal route source row stride overflow")?;
    let source_payload_bytes = args
        .rows
        .checked_mul(source_row_stride_bytes)
        .context("B12X terminal route source payload overflow")?;
    let source_payload = upload(
        library,
        &vec![0_u8; source_payload_bytes],
        "B12X terminal route NVFP4 source payload",
    )?;
    let row_indices = (0..args.rows)
        .map(|row| u32::try_from(row).context("B12X terminal route row index exceeds u32"))
        .collect::<Result<Vec<_>>>()?;
    let row_indices = upload(
        library,
        u32_bytes(&row_indices),
        "B12X terminal route row indices",
    )?;
    let route_weights = upload(
        library,
        f32_bytes(&vec![1.0_f32; args.rows]),
        "B12X terminal route weights",
    )?;
    let accumulator_values = args
        .rows
        .checked_mul(args.output_dim)
        .context("B12X terminal route accumulator value count overflow")?;
    let accumulator = DeviceAllocation::new(
        library,
        accumulator_values * std::mem::size_of::<f32>(),
        "B12X terminal route accumulator",
    )?;
    let response_row_stride_bytes = args
        .output_dim
        .checked_add(std::mem::size_of::<f32>())
        .context("B12X terminal route response row stride overflow")?;
    let response_bytes = args
        .rows
        .checked_mul(response_row_stride_bytes)
        .context("B12X terminal route response byte count overflow")?;
    let response_output =
        DeviceAllocation::new(library, response_bytes, "B12X terminal route FP8 response")?;
    let response_host = HostAllocation::new(
        library,
        response_bytes,
        "B12X terminal route FP8 response D2H",
    )?;

    for stream_count in stream_counts {
        let serial_timing = time_kernel(args, || {
            for expert in experts.iter().take(stream_count) {
                unsafe {
        ...
        emit_timing(
            ...
            &json!({
                ...
                "input_dtype": "nvfp4",
                "output_dtype": "bf16",
                "distinct_expert_weights": true,
                "expert_mlps_per_ms": stream_count as f64 / serial_timing.avg_ms,
                "average_ms_per_expert": serial_timing.avg_ms / stream_count as f64,
            }),
            args,
            serial_timing,
        )?;

        let serial_graph = unsafe {
            streams[0].synchronize()?;
            library
                .cuda_graph_begin_capture(streams[0].raw())
                .context("beginning B12X AOT serial expert MLP graph capture")?;
            for expert in experts.iter().take(stream_count) {
                library.cuda_b12x_spark_mlp_prequantized_async(
                    &expert.buffers,
                    args.rows,
            ...
            BenchCudaGraph::new(
                library,
                library
                    .cuda_graph_end_capture_retained(streams[0].raw())
                    .context("ending B12X AOT serial expert MLP graph capture")?,
            )?
        };
        let serial_graph_timing = time_kernel(args, || unsafe {
            library
                .cuda_graph_launch(serial_graph.graph_exec(), streams[0].raw())
                .context("launching B12X AOT serial expert MLP graph")?;
            streams[0].synchronize()
        })?;
        emit_timing(
            native_lib,
            ...
            &json!({
                ...
                "output_dtype": "bf16",
                "distinct_expert_weights": true,
                "graph_nodes": serial_graph.node_count(),
                "graph_kernel_nodes": serial_graph.kernel_node_count(),
                "expert_mlps_per_ms": stream_count as f64 / serial_graph_timing.avg_ms,
                "average_ms_per_expert": serial_graph_timing.avg_ms / stream_count as f64,
                "speedup_vs_serial": serial_timing.avg_ms / serial_graph_timing.avg_ms,
            }),
            args,
            serial_graph_timing,
        )?;

        let concurrent_timing = time_kernel(args, || {
            for (stream, expert) in streams.iter().zip(&experts).take(stream_count) {
                unsafe {
                    library.cuda_b12x_spark_mlp_prequantized_async(
                        &expert.buffers,
                        args.rows,
            ...
            for stream in streams.iter().take(stream_count) {
                unsafe { stream.synchronize()? };
            }
            Ok(())
        })?;
        emit_timing(
            ...
            &json!({
                ...
                "output_dtype": "bf16",
                "distinct_expert_weights": true,
                "expert_mlps_per_ms": stream_count as f64 / concurrent_timing.avg_ms,
                "average_ms_per_expert": concurrent_timing.avg_ms / stream_count as f64,
                "speedup_vs_serial": serial_timing.avg_ms / concurrent_timing.avg_ms,
            }),
            args,
            concurrent_timing,
        )?;

        let serial_terminal_timing = time_kernel(args, || unsafe {
            library.cuda_zero_f32_async(
                accumulator.buffer(),
                accumulator_values,
                streams[0].raw(),
            )?;
            for expert in experts.iter().take(stream_count) {
                library.cuda_b12x_prepare_nvfp4_row_payload_async(
                    source_payload.buffer(),
                    args.rows,
                    source_row_stride_bytes,
                    row_indices.buffer(),
                    expert.buffers.input_packed,
                    expert.buffers.input_scale,
                    args.rows,
                    args.hidden_dim,
                    streams[0].raw(),
                )?;
                ...
                library.cuda_scatter_add_rows_bf16_weighted_to_f32_async(
                    expert.buffers.output,
                    row_indices.buffer(),
            ...
            library.cuda_gather_rows_f32_to_fp8_e4m3_row_scaled_async(
                ...
                args.rows,
                args.output_dim,
                response_row_stride_bytes,
                streams[0].raw(),
            )?;
            library.copy_d2h_host_buffer_async(
                response_host.buffer(),
                response_output.buffer(),
                response_bytes,
                streams[0].raw(),
            )?;
        ...
        emit_timing(
            native_lib,
            "b12x_spark_aot_terminal_route_fp8",
            &json!({
                "rows": args.rows,
                "expert_count": stream_count,
                "execution": "serial",
                ...
                "output_dim": args.output_dim,
                "request_dtype": "nvfp4",
                "response_dtype": "fp8_e4m3_row_scaled",
                "response_bytes": response_bytes,
            }),
            args,
            serial_terminal_timing,
        )?;

        let concurrent_terminal_timing = time_kernel(args, || unsafe {
            library.cuda_zero_f32_async(
                accumulator.buffer(),
                accumulator_values,
                streams[0].raw(),
            )?;
            library.cuda_event_record(zero_ready.raw(), streams[0].raw())?;
            for stream in streams.iter().take(stream_count).skip(1) {
                library.cuda_stream_wait_event(stream.raw(), zero_ready.raw())?;
            }
            for ((stream, expert), ready) in streams
                .iter()
                .zip(&experts)
                .zip(&expert_ready)
                .take(stream_count)
            {
                ...
                library.cuda_event_record(ready.raw(), stream.raw())?;
                library.cuda_stream_wait_event(completion_stream.raw(), ready.raw())?;
            }
            library.cuda_gather_rows_f32_to_fp8_e4m3_row_scaled_async(
                accumulator.buffer(),
                args.rows,
        ...
        emit_timing(
            ...
            &json!({
                ...
                "response_dtype": "fp8_e4m3_row_scaled",
                "response_bytes": response_bytes,
                "speedup_vs_serial": serial_terminal_timing.avg_ms / concurrent_terminal_timing.avg_ms,
            }),
            args,
            concurrent_terminal_timing,
        )?;
    }
    Ok(())
}
...
impl<'a> BenchCudaGraph<'a> {
    fn new(library: &'a NativeLibrary, capture: GlmrtCudaGraphCaptureInfo) -> Result<Self> {
        if capture.graph.is_null() || capture.graph_exec.is_null() {
            anyhow::bail!("b12x Spark dense GEMM graph capture returned a null graph handle");
        }
        if capture.kernel_node_count == 0 {
            anyhow::bail!("b12x Spark dense GEMM graph capture did not include a kernel node");
        }
        Ok(Self {
            library,
            graph: capture.graph,

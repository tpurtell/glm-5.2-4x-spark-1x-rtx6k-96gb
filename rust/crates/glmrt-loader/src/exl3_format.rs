...
pub(crate) fn exl3_recipe_from_quantization_config(
    ...
    for field in ["quant_method", "method", "format", "checkpoint_format"] {
        anyhow::ensure!(
            ...
            "GLM-5.2 EXL3 requires quantization_config.{field}=exl3"
        );
    }
    anyhow::ensure!(
        value.get("bits").and_then(Value::as_f64) == Some(GLM52_EXL3_BITS as f64),
        "GLM-5.2 EXL3 requires an exact K3 payload"
    );
    anyhow::ensure!(
        ...
        "GLM-5.2 EXL3 requires the K3 MCG/auto-scale storage contract"
    );
    let includes = value
        .get("module_include")
        .and_then(Value::as_array)
        .context("GLM-5.2 EXL3 requires module_include")?;
    anyhow::ensure!(
        includes.len() == 1 && includes[0].as_str() == Some(BASE_EXPERT_MODULE_PATTERN),
        "GLM-5.2 EXL3 must select exactly the routed base experts in layers 3 through 77"
    );
    anyhow::ensure!(
        value
            .get("tensor_storage")
        ...
        "GLM-5.2 EXL3 tensor_storage cannot be empty"
    );
    Ok(Some(GLM52_EXL3_RECIPE_K3_V1))
}
...
impl Glm52Exl3Tp4ResidentGeometry {
    pub fn from_model_facts(facts: &ModelFacts) -> Result<Self> {
        anyhow::ensure!(
            is_glm52_exl3_recipe(&facts.quantization_recipe),
            "GLM-5.2 EXL3 TP4 geometry requires recipe {GLM52_EXL3_RECIPE_K3_V1}, got {:?}",
        ...
        let projection_trellis_bytes = hidden
            ...
            .and_then(|values| values.checked_mul(GLM52_EXL3_BITS as u64))
            .and_then(|bits| bits.checked_div(8))
            .context("GLM-5.2 EXL3 projection size overflow")?;
        let w13_trellis_bytes = projection_trellis_bytes
            .checked_mul(experts)
            .and_then(|bytes| bytes.checked_mul(2))
            .context("GLM-5.2 EXL3 W13 size overflow")?;
        let w2_trellis_bytes = projection_trellis_bytes
            .checked_mul(experts)
            .context("GLM-5.2 EXL3 W2 size overflow")?;
        let hidden_rotation_table_bytes = experts
            .checked_mul(hidden)
            .and_then(|values| values.checked_mul(2))
            .context("GLM-5.2 EXL3 hidden rotation size overflow")?;
        let intermediate_rotation_bytes = experts
            .checked_mul(3)
            .and_then(|values| values.checked_mul(local_intermediate))
            .and_then(|values| values.checked_mul(2))
            .context("GLM-5.2 EXL3 intermediate rotation size overflow")?;
        ...
        let scalar_metadata_bytes = facts
            ...
            .context("GLM-5.2 EXL3 unit global scale size overflow")?
            as u64;
        Ok(Self {
            trellis_bits: GLM52_EXL3_BITS,
...
pub fn glm52_exl3_expert(
    ...
    anyhow::ensure!(
        ...
        "GLM-5.2 EXL3 layer {layer_id} is outside the base routed interval"
    );
    anyhow::ensure!(
        expert_id < catalog.facts.routed_experts,
        "GLM-5.2 EXL3 expert {expert_id} exceeds {} routed experts",
    ...
    let projection = |kind: Glm52Exl3ProjectionKind| -> Result<Glm52Exl3Projection<'_>> {
        ...
        let find = |suffix: &str| -> Result<&TensorInfo> {
            ...
            catalog
                ...
                .with_context(|| format!("missing GLM-5.2 EXL3 tensor {name}"))
        };
...
pub fn validate_glm52_exl3_expert_catalog(
    ...
    anyhow::ensure!(
        ...
        "GLM-5.2 EXL3 validation requires recipe {GLM52_EXL3_RECIPE_K3_V1}, got {}",
    ...
    for layer_id in catalog.facts.first_k_dense_replace..catalog.facts.num_hidden_layers {
        for expert_id in 0..catalog.facts.routed_experts {
            let expert = glm52_exl3_expert(catalog, layer_id, expert_id).with_context(|| {
                format!("validating GLM-5.2 EXL3 layer {layer_id} expert {expert_id}")
            })?;
            for projection in [expert.gate, expert.up, expert.down] {
                ...
                trellis_bytes = trellis_bytes
                    ...
                    .context("GLM-5.2 EXL3 trellis total overflow")?;
                rotation_bytes = rotation_bytes
                    ...
                    .context("GLM-5.2 EXL3 rotation total overflow")?;
            }
        }
    }
    ...
    anyhow::ensure!(
        ...
        "GLM-5.2 EXL3 routed tensor set mismatch: expected {}, found {}; missing={:?}; unexpected={:?}",
        expected_names.len(),
        actual_names.len(),
    ...
    anyhow::ensure!(
        ...
        "GLM-5.2 EXL3 geometry changed while validating the catalog"
    );
...
fn validate_projection(projection: Glm52Exl3Projection<'_>) -> Result<()> {
...
fn validate_tensor(
    ...
    anyhow::ensure!(
        ...
        "invalid GLM-5.2 EXL3 tensor {}: role={:?} dtype={:?} shape={:?} bytes={} metadata={}; expected dtype={:?} shape={:?} bytes={expected_bytes} metadata={expected_quantization_metadata}",
        tensor.name,
        tensor.role,
        tensor.dtype,
        tensor.shape,
...
mod tests {
    ...
    fn one_layer_exl3_catalog() -> TensorCatalog {
            ...
            for (projection, input_features, output_features) in [
                ...
                for (suffix, dtype, shape, metadata) in [
                    (
                        "trellis",
                        DType::I16,
                        vec![input_features / 16, output_features / 16, 48],
                        false,
                    ),
        ...
        TensorCatalog {
            ...
            snapshot_path: "/test/glm52-exl3".to_owned(),
            facts,
            tensors,
        }
    }
    ...
    #[test]
    fn exact_k3_config_is_recognized_without_weakening_nvfp4() {
    ...
    #[test]
    fn rank_local_geometry_is_compact_k3_without_dual_residency() {
        let mut facts = ModelFacts::default();
        facts.quantization_recipe = GLM52_EXL3_RECIPE_K3_V1.to_owned();
    ...
    fn complete_exl3_layer_requires_exact_native_tensor_namespace() {
        let mut catalog = one_layer_exl3_catalog();

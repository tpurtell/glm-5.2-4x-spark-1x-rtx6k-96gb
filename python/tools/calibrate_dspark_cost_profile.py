...
def render_rust(profile: dict[str, Any], source_sha256: str) -> str:
    identity = profile["identity"]
    lines = [
        "// @generated by python/tools/calibrate_dspark_cost_profile.py; do not edit.",
        f'pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_ID: &str = {json.dumps(profile["profile_id"])};',
        f'pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_SOURCE_SHA256: &str = "{source_sha256}";',
        f'pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_TARGET_MODEL: &str = {json.dumps(identity["target_model"])};',
        f'pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_TARGET_REVISION: &str = {json.dumps(identity["target_revision"])};',
        f'pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_DSPARK_MODEL: &str = {json.dumps(identity["dspark_model"])};',
        f'pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_DSPARK_REVISION: &str = {json.dumps(identity["dspark_revision"])};',
        f'pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_SPARKINFER_REVISION: &str = {json.dumps(identity["sparkinfer_revision"])};',
        f'pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_TOPOLOGY: &str = {json.dumps(identity["topology"])};',
        f"pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_POWER_LIMIT_WATTS: usize = {identity['power_limit_watts']};",
        f"pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_MAX_CONCURRENCY: usize = {identity['max_concurrency']};",
        f"pub(super) const GLM52_REDHAT_DSPARK_COST_PROFILE_MAX_DRAFTS: usize = {identity['max_drafts']};",
        "pub(super) static GLM52_REDHAT_DSPARK_COST_PROFILE_MS: [&[(usize, f64)]; GLM52_REDHAT_DSPARK_COST_PROFILE_MAX_CONCURRENCY] = [",
    ]
...
def main() -> None:
    ...
    if args.output_rust is not None:
        ...
        args.output_rust.write_text(render_rust(profile, source_sha256), encoding="utf-8")
    print(
        json.dumps(
            {
